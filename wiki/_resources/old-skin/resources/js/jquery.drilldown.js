...
(function($){
	...
	var discover = this,
		...
		settings = {
			...
			filters:[],
			templates:{
				filter_header:'<h3 class="dropdown-filter-header"></h3>',
				printouts:{},
				no_results: '<div>We couldn\'t find any plants with the properties you searched for.</div>',
				more_results: '<div>There are more results available than displayed below. Please further refine your search!</div>',
				result_count: 'Your search returned $1 plants!'
			}
		},
		options = {
	...
	var filter_types = {
		...
		propertyValues: function(filter){
			...
			var el = $('<div class="dropdown-filter dropdown-filter-propertyvalues"></div>'),
				f = filter,
				state = [];
			...
			var events = {
				inputChange: function(ev){
					...
					var $this = $(this);
					console.log('Input change: ',$this.data('property'),$this.val());
					console.log($this.val(),f.values,f);
			...
			this.draw = function(){
				el.empty();
				el.append( $(options.templates.filter_header).text(f.label!==undefined ? f.label : f.property) );
				for(var i=0, l=f.values.length, label;i<l;i++){
					...
					el.append($('<label class="dropdown-filter-checkbox"></label>').append(input).append(label));
				}
				return el;
			}
		...
		singleProperty: function(f){
			...
			this.draw = function(){
				...
				input = $('<input type="checkbox">')
						...
						.val(value)
						.change(events.inputChange);
				return $('<label class="dropdown-filter-checkbox"></label>').append(input).append(label);
			}
			
			this.getConditions = function(){
				var value = f.value!==undefined ? f.value : '+';
				if(state===true){
		...
		propertyExists: function(f){
			...
			var el = $('<div class="dropdown-filter dropdown-filter-propertyexists"></div>'),
				state = false,
			...
			this.draw = function(){
				...
				var label = f.label!==undefined ? f.label : f.property,
					input;
				el.append( $(options.templates.filter_header).text(label) );
				el.append( filter.draw() );
				return el;
			}
	...
	var methods = {
		...
		init: function(opts){
			...
			for(var i=0,l=options.filters.length, f;i<l;i++){ 
				...
				//keep track of state of checkboxes
				/*if(filters[i].state===undefined){
					if(filters[i].values!==undefined){
						filters[i].state = [];
						for(i2=0,l2=filters[i].values.length;i2<l2;i2++){
							filters[i].state.push(false);
						}
					}else{
		...
		drawUpdate: function(results){
			...
			var item, printout, printouts, items=[], result, print_out_values, i, l;
			
			for(var key in results){
				result = results[key];
				item = $('<div class="discover-result-item"></div>');
				item.append( '<a href="'+result.fullurl+'">'+result.fulltext+'</a>' );
				if(typeof result.printouts=='object'){
					printouts = $('<div class="discover-printouts"></div>');
					for(key in result.printouts){
						...
						printout = $('<div class="discover-printout discover-printout-'+key.toLowerCase().replace(/( )/g,'_')+'"></div>');
						switch(typeof options.templates.printouts[key]){
							case 'string':
								printout.append( $(options.templates.printouts[key]).addClass('discover-printout').text( print_out_values.join(', ') ) );
								break;
							case 'object':
								if(options.templates.printouts[key].display_as!==undefined){
									switch(options.templates.printouts[key].display_as){
										case 'image':
											if(print_out_values.length>0)
												printout.append('<img src="'+print_out_values[0]+'">');
											break;
									}
								}
								break
							case 'undefined':
							default:
								printout.text(print_out_values.join(', '));
							break;
						}
			...
			if(items.length === options.query.limit){
				elements.info.append(options.templates.more_results);
			}else if(items.length===0){
				elements.info.append(options.templates.no_results);
			}else{
				elements.info.append(options.templates.result_count.replace(/\$1/g,items.length));
			}
		},
	...
	$.fn.discover = function( method ) {
	  	console.log('discover called with',arguments);
